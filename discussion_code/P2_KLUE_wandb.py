...
wandb.run.name = 'skt-kobert-both-loss-seed-7'#cfg['run_name']
...
config.smoothing_rate = 0.2
loss_labelsmoothing = LabelSmoothingLoss(smoothing=config.smoothing_rate)
...
# Model path
model_path = '/opt/ml/model/model_{}.pt'.format(wandb.run.name)
...
for e in range(num_epochs):
    ...
    for batch_id, (token_ids, valid_length, segment_ids, label) in enumerate(train_dataloader):
        ...
        loss = loss_labelsmoothing(out, label) + loss_focal(out, label)
        loss.backward()
        ...
        if batch_id % log_interval == 0:
            ...
            wandb.log({"loss": loss.data.cpu().numpy(), "train_accuracy": train_acc / (batch_id+1)})
...
sub_name = model_path.split("/")[-1].split('.')[0]
output.to_csv('/opt/ml/submission_{}.csv'.format(sub_name), index=False)
np.save('/opt/ml/logits/{}.npy'.format(sub_name), logits)